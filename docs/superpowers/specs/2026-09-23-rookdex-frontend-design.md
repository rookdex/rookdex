...
- No change to the Tracker island's data model, profile menu or delete flow.
...
- **Reveal.** A module script (about 1 KB) removes `hidden` when `matchMedia("(display-mode: standalone)")` matches, or when iOS reports `navigator.standalone`. A browser tab never shows the buttons.
- **State.** Back is disabled on the first page of the session; forward is disabled when `navigation.canGoForward` is false. Disabled buttons stay in place, dimmed (`opacity: .45`, muted colour), so the mark never shifts. Where the Navigation API is missing, both stay enabled and call `history.back()` / `history.forward()`. The script keeps no history of its own and stores nothing.
...
One `<nav aria-label="Main">` (Norwegian: "Hoved") placed after the header and before `<main>`, holding `<ul>` › `<li>` › `<a>`. The current tab gets `aria-current="page"`, computed at build from the first segment of the `path` prop `Base.astro` already takes: `""` is Home, `tracker` is Tracker, `news` is News, `settings` is Settings, and anything else (guides) marks no tab. Zero JavaScript; it works on a cold cache and with scripts off.

**Phone (below 768 px): fixed bottom bar.**

- Four equal columns (`repeat(4, 1fr)`), each link a column of icon over label, 56 px tall, 4 px gap, label 12 px / 500, `white-space: nowrap`, line-height 1.
- Bar height 56 px plus `env(safe-area-inset-bottom)`. The viewport meta becomes `width=device-width, initial-scale=1, viewport-fit=cover`. `body` gets bottom padding equal to the bar so the footer is never covered.
...
- Norwegian "Innstillinger" measures 66.7 px in an 80 px cell at 100 % but about 87 px at 130 %. Its copy string carries a soft hyphen (`Inn­stillinger`) so it breaks onto a second line instead of clipping; the bar grows (measured 65 → 68 px) rather than cutting text. This is the one label allowed to wrap.
...
- Script (about 1 KB, progressive): Escape closes and returns focus to the summary; a tap outside closes; up and down arrows move between links; opening moves focus to the current language. Without the script, `<details>` still opens and closes.
...
| Protected from clearing / Beskyttet mot sletting | Yes / Ja, No / Nei — from `navigator.storage.persisted()`; the row is left out when the API is missing |
...
The button opens a native `<dialog>` that repeats the consequence, with "Delete everything" / "Slett alt" and "Cancel" / "Avbryt". No type-to-confirm. On confirm the script:

1. Calls `indexedDB.deleteDatabase("rookdex")`.
2. Removes `rookdex.install-prompt-seen` and `rookdex.persist-hint-seen` from `localStorage`.
3. Leaves the service worker cache alone, so the app still opens offline.
4. On success, closes the dialog and announces "All data on this device is deleted." / "Alle data på denne enheten er slettet." in a polite live region.
5. On `blocked` (another Rookdex tab holds the database open), keeps the dialog open and says "Close other Rookdex tabs, then try again." / "Lukk andre Rookdex-faner og prøv igjen."
6. On `error`, keeps the dialog open with "Something went wrong. Nothing was deleted." / "Noe gikk galt. Ingenting ble slettet."

### 7.3 App

| Label | Value |
|---|---|
| Install / Installer | Button "Install Rookdex" / "Installer Rookdex", shown only after `beforeinstallprompt`. "Installed" / "Installert" in standalone. On iOS Safari (detected by `navigator.standalone` being defined and false, no user-agent sniffing): "Share, then Add to Home Screen" / "Del, deretter Legg til på Hjem-skjerm". The row is left out when none applies |
...
- The delete dialog uses native `<dialog>` with `showModal()`, so focus is trapped and Escape cancels without extra code.
...
- The CSP stays as it is. Every script is an Astro-processed module served from `'self'`; no inline handlers, no `'unsafe-inline'`.
...
| Unit | Checks |
|---|---|
| `currentTab(path)` | `""` → home, `tracker` → tracker, `news` → news, `settings` → settings, `guides/x` → none |
| `wireHistoryButtons(group, win)` | Hidden unless standalone; back disabled on first entry; forward follows `canGoForward`; without the Navigation API both stay enabled and call `history` |
| `wireLanguageMenu(details)` | Escape closes and refocuses the summary; outside click closes; arrows move; open focuses the current link |
| `deleteAllData(idb, storage)` | Deletes the `rookdex` database and both keys, leaves others; reports `blocked` and `error` distinctly (uses `fake-indexeddb`) |
| Settings rows | Storage row omitted without the API; install row states (prompt, installed, iOS how-to, none) |
...
| Copy | New keys exist in both locales; tab labels other than Settings contain no soft hyphen |
...
**Measured before merge.** A Browser-pane script at 320, 360, 768 and 1024 px, each at 100 % and 130 % text: header row width, tab-bar label fit, footer grid (one line per value, shared baselines), sources line. Then my phone check on the version preview: back and forward in the installed app, the safe-area gap under the bar, and the delete-all flow.
